...
public class UploadUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UploadUser</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UploadUser at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (update.equalsIgnoreCase("UpdateProfile")) {
            ...
            if (create != null) {
                ...
                //set session new ai
                request.setAttribute("accountinfo", newai);
            ...
            request.setAttribute("account", newa);
            AccountInfo ai = d.getAccountInfo(a);
            AccountImage am = d.getAccountImage(a);
            if (am != null) {
                request.setAttribute("accountImage", base64image(am.getImage()));
        ...
        request.getRequestDispatcher("accountinfo?aid=" + aid).forward(request, response);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO d = new DAO();
        PrintWriter out = response.getWriter();
        String add = request.getParameter("aid");
        int aid = 0;
        try {
            aid = Integer.parseInt(add);
        } catch (Exception e) {
        }
        Account a = d.getAccountFromAid(aid);
        ...
        if (update.equalsIgnoreCase("UpdateProfile")) {
            request.getRequestDispatcher("settinguser.jsp?&update=1").forward(request, response);
        } else {
            request.getRequestDispatcher("settinguser.jsp?&update=2").forward(request, response);
        }
    }
